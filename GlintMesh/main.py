...
from database import connect_db, close_db, users_collection, datasets_collection, MONGODB_AVAILABLE
from socket_events import (
    sio, emit_generation_start, emit_tool_call, emit_tool_result,
...
async def run_agent_stream(user_message: str, lang: str = "es", context: str | None = None,
    ...
    await emit_generation_start({'message': user_message, 'mode': mode})
...
async def auth_register(body: AuthBody):
    ...
    if MONGODB_AVAILABLE:
        existing = await users_collection().find_one({"username": key})
        if existing:
            raise HTTPException(status_code=409, detail="User already exists.")
        await users_collection().insert_one({
            "username": key, "display_user": body.user, "salt": salt,
            "hash": hashed, "sessions": [], "avatar": None
        })
    else:
        users = _load_users_file()
        if key in users:
            raise HTTPException(status_code=409, detail="User already exists.")
        users[key] = {"user": body.user, "salt": salt, "hash": hashed, "sessions": []}
...
async def auth_login(body: AuthBody):
    ...
    if MONGODB_AVAILABLE:
        record = await users_collection().find_one({"username": key})
        if not record or record.get("hash") != _hash_password(body.password, record.get("salt", "")):
            raise HTTPException(status_code=401, detail="Invalid credentials.")
        token = uuid.uuid4().hex + uuid.uuid4().hex
        await users_collection().update_one({"username": key}, {"$push": {"sessions": token}})
        return {"user": record.get("display_user", record["username"]), "token": token, "avatar": record.get("avatar")}
    else:
        users = _load_users_file()
...
def _auth_record_sync(request: Request) -> dict | None:
...
async def _auth_record_async(request: Request) -> dict | None:
    auth = request.headers.get("authorization", "")
    token = auth[7:] if auth.lower().startswith("bearer ") else ""
    if not token:
        return None
    if MONGODB_AVAILABLE:
        return await users_collection().find_one({"sessions": token})
    users = _load_users_file()
    for record in users.values():
        if token in record.get("sessions", []):
            return record
...
async def auth_me(request: Request):
    record = await _auth_record_async(request)
    if record:
        user = record.get("display_user", record.get("user", record.get("username")))
        return {"user": user, "avatar": record.get("avatar")}
    return {"user": None}
...
async def auth_avatar(body: AvatarBody, request: Request):
    ...
    if MONGODB_AVAILABLE:
        record = await users_collection().find_one({"sessions": token})
        if not record:
            raise HTTPException(status_code=401, detail="Not signed in.")
        await users_collection().update_one(
            {"username": record["username"]}, {"$set": {"avatar": body.avatar[:400000]}}
        )
        return {"user": record.get("display_user", record["username"]), "avatar": body.avatar[:400000]}
    else:
        users = _load_users_file()
...
async def auth_logout(body: dict):
    ...
    if MONGODB_AVAILABLE:
        await users_collection().update_one({"sessions": token}, {"$pull": {"sessions": token}})
    else:
        users = _load_users_file()
        for record in users.values():
...
async def list_datasets():
    items = []
    if MONGODB_AVAILABLE:
        async for ds in datasets_collection().find({}):
            items.append({"id": ds["dataset_id"], "name": ds["name"], "columns": ds["columns"],
                          "n_rows": ds["n_rows"], "size": ds["size"], "sample": ds["sample"]})
    else:
        for path in sorted(UPLOAD_DIR.glob("*")):
            ...
            try:
                ...
                continue
            items.append({"id": path.stem, "name": path.name, "columns": columns,
                          "n_rows": len(rows), "size": path.stat().st_size, "sample": rows[:3]})
...
async def upload_dataset(file: UploadFile = File(...)):
    ...
    temp_path = UPLOAD_DIR / f"{dataset_id}{ext}"
    temp_path.write_bytes(raw)
    try:
        columns, rows = _parse_dataset_rows(temp_path)
    except (ValueError, json.JSONDecodeError, UnicodeDecodeError):
    ...
    if MONGODB_AVAILABLE:
        await datasets_collection().update_one(
            {"dataset_id": dataset_id}, {"$set": {
                "dataset_id": dataset_id, "name": file.filename or f"{dataset_id}{ext}",
                "columns": columns, "rows": rows, "n_rows": len(rows), "size": len(raw),
                "sample": rows[:5]
            }}, upsert=True
        )

    return {"id": dataset_id, "name": file.filename, "columns": columns, "n_rows": len(rows),
...
async def health():
    return {"status": "ok", "service": "GlintMesh", "version": "2.0.0",
            ...
            "mcp_enabled": MCP_ENABLED, "mongodb_enabled": MONGODB_AVAILABLE,
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:socket_app", host="127.0.0.1", port=8080, reload=True)